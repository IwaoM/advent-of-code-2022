class Cpu {
  ...
  tick (newVal) {
    ...
    this.register.push(newVal ? newVal : this.register[this.register.length - 1]); // cycle ends : register is updated
  }
...
module.exports = function day10 (inputData) {
  ...
  for (let i = 0; i < inputArray.length; i++) {
    if (inputArray[i] === "noop") {
      ...
      part1Cpu.addX(parseInt(inputArray[i].split(" ")[1]));
    }
  }
  let result1 = 0;
  for (let i = 20; i <= 220; i += 40) {
    result1 += part1Cpu.registerDuringCycle(i) * i;
  }
